...
>
> 当前核心目标：先把可重复的 `CF 资源 → Source 1 → 编译 → MIGI → CS:GO` 流水线做稳定，再替换为准确的最终雷神资产。

本文是项目唯一的 authoritative progress/status。README 负责工具说明和证据索引；旧 A1–F4 名称只用于关联已有报告，不再各自维护另一套进度。
...
1. **Source 1 官方基线与编译闭环**
   ...
   - 早期官方 M4A1-S A3 基线只保留为工具链证据；当前运行目标已经统一为 M4A4。
...
2. **CF 静态枪模进入 Source 1**
   ...
   - 9 个枪体 mesh 已进入 M4A4 viewmodel：主体→Parent、01→Clip、02→Bolt、03–08→Parent Prototype 静态降级。
...
3. **动作与机械件 Prototype 闭环**
   ...
   - 当前不实现 M4A1-S 消音器、消音器装卸状态或对应骨骼；这条旧路线已退出 active plan。

4. **材质引用闭环**
   - SMD → QC `$cdmaterials` → VMT → VTF 的引用验证器已经成立。
   - 当前可识别材质来自网络第三方移植包，只作为 `REFERENCE/PROTOTYPE` 使用；它证明 UV、VTF/VMT、自发光和 MIGI 材质链可运行，不代表最终 CF 雷神材质完成。
...
当前不再继续猜目标枪，也不因 B1/B2/C2 技术债推翻已经成立的 Prototype。工作重点是把已有脚本串成稳定、可重复、失败即停止的通用流水线：
...
- 用最终本地 CF 资产替换 Prototype 输入并重跑同一流水线；
...
### 当前 Prototype 与“最终雷神”的区别

| 项目 | 当前 Prototype | 最终雷神 |
|---|---|---|
| 运行槽位 | M4A4 | M4A4 |
| 模型输入 | 本地 CF 候选 LTB；身份暂不作最终承诺 | 经参考图/纹理/几何反查确认的本地 CF 原始 LTB |
| 动作 | 官方 M4A4 动作 | 官方 M4A4 fallback 或经证明的 CF 原动作 |
| 材质 | 网络第三方贴图，仅用于可识别与链路验证 | 本地 CF DTX/TGA/CFG 转换结果 |
| 分件绑定 | 主体/Clip/Bolt 已工作；03–08 为 Prototype Parent fallback | 每个动态件有最终语义和正确骨骼绑定 |
...
Track B 未完成时，禁止把 Prototype 改名为“最终雷神”，但也禁止因此回退 Track A 的技术闭环。
...
- 网络第三方贴图：可放在 `work/.../reference` 或 Prototype build 中验证链路，但必须标记来源、哈希和 `final=false`。
...
- 在 manifest/report 中省略外部来源或把 Prototype 状态写成 final；
- 为了追求当前枪型身份准确而删除已经通过的构建/MIGI 证据。

当前 F4 网络材质必须继续保持 `REFERENCE/PROTOTYPE`；最终 F 阶段仍是未完成。

---

## 4. 简化后的阶段划分

### P0：Source 1 基线与安全构建 — COMPLETE / FROZEN

成果：官方参考提取、反编译、编译和回环报告；隔离 build/work；显式 MIGI deploy；M4A4 runtime contract 已确定。

旧 A1/A2/A3 映射到本阶段。官方 M4A1-S 产物只作为历史工具验证，不是 active runtime。

### P1：CF 静态资源导出 — PROTOTYPE COMPLETE / DEBT RECORDED
...
旧 B1/B2/B3/C1 映射到本阶段。rigid bone index、bind matrix 独立证据和 CF clip 解码仍是技术债，但不阻塞 Prototype。

### P2：M4A4 Source 映射 — PROTOTYPE COMPLETE / FROZEN

成果：M4A4 57-bone reference；M4A4 专用坐标/比例变换；主体→Parent、01→Clip、02→Bolt、03–08→Parent fallback；QC attachment 和官方动作兼容。

旧 C2/C3/D1/D2 映射到本阶段。M4A1-S 消音器路线不再进入 active gate。

### P3：编译、材质引用、MIGI 与实机 — PROTOTYPE COMPLETE / FROZEN

成果：9 mesh viewmodel 编译；官方 M4A4 sequence 保留；VMT/VTF 引用闭包；MIGI addon 生成与逐文件一致性；游戏内模型、贴图和动作可见。

旧 D3/F1–F4 映射到本阶段。F4 只代表 Prototype 可识别材质，不代表最终 F 材质完成。

### P4：通用流水线稳定化 — CURRENT

退出条件：

- 一个 authoritative manifest 能完整描述 Prototype 输入和输出；
...
### P6：最终资产替换与发布质量 — LATER

退出条件：只替换 manifest 输入即可重跑 P4；最终本地 CF mesh/UV/material 在游戏中正确；动态件、attachments、动作和材质无未说明的 Prototype fallback；final addon 不依赖网络 MOD 文件。
...
- 只保留一个 Prototype 构建 manifest 作为入口；现有 mesh/alignment/material JSON 作为被引用的证据文件，不各自声明总进度。
...
| 技术债 | 当前判断 | 是否阻塞 P4 Prototype | 何时修 |
|---|---|---:|---|
...
| 当前 F4 材质来自第三方 MOD | 不能进入 final 发布 | 否 | P5/P6 用本地 CF 材质替换 |
| 世界/掉落模型仍是官方 M4A4 | 第一人称闭环不受影响 | 否 | P7 |

修债原则：只修会影响流水线复现、第二种武器复用或最终资产替换的问题；不得以“技术债存在”为由重写已通过的 Prototype 编译/MIGI 闭环。
...
- Silencer：不支持、不拆装、不进入当前验收。
- Main binding：`v_weapon.M4A1_Parent`。
- Magazine binding：`v_weapon.M4A1_Clip`。
- Bolt binding：`v_weapon.M4A1_Bolt`。
- 03–08：Prototype 阶段 Parent fallback，明确非最终语义。
- CF 57-node skeleton：用于理解源资源和未来 CF 动画，不进入当前使用官方动作的 Source viewmodel skeleton。
- 当前网络贴图：只作为 Prototype/reference；`final_cf_material=false`。

任何未来修改若改变以上决定，必须新建 manifest/profile，不得悄悄复活 M4A1-S/silencer 旧路线。

---

## 8. 两套 Definition of Done

### 8.1 流水线 Prototype DoD
...
- 所有 Prototype fallback 已替换，或作为明确限制写入发布说明；
...
下一次实现应从 **P4 Manifest 收敛** 开始，而不是继续调当前枪型外观，也不是回到 M4A1-S/silencer 路线。
