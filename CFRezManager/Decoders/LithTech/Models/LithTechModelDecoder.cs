...
public sealed record LithTechModelDocument(
    ...
    IReadOnlyList<LithTechSkeletonNode>? SkeletonNodes = null)
{
...
public sealed record LithTechMesh(
    ...
    IReadOnlyList<LithTechVector3>? Normals = null,
    IReadOnlyList<LithTechVertexWeights>? BoneWeights = null,
    IReadOnlyList<LithTechVertexBoneIndices>? BoneIndices = null)
{
    ...
    public bool HasBoneWeights => BoneWeights is not null && BoneWeights.Count == Vertices.Count;

    public bool HasBoneIndices => BoneIndices is not null && BoneIndices.Count == Vertices.Count;
}
...
public sealed record LithTechSkeletonNode(
    int Index,
    string Name,
    int ParentIndex,
    int ChildCount,
    IReadOnlyList<double> BindMatrix);

/// <summary>
/// The Jupiter LTB vertex stream stores up to three weight values alongside a
/// skinned vertex.  The corresponding bone-index lookup table is decoded
/// separately from the per-vertex-range section.
/// </summary>
public readonly record struct LithTechVertexWeights(double First, double Second, double Third)
{
    public double Sum => First + Second + Third;
}

/// <summary>
/// Four one-byte node indices are packed into two uint16 values in the LTB
/// per-vertex-range table.  255 is the empty influence sentinel.
/// </summary>
public readonly record struct LithTechVertexBoneIndices(int First, int Second, int Third, int Fourth)
{
    public int ValidCount => new[] { First, Second, Third, Fourth }.Count(index => index != 255);
}
...
internal static class LithTechModelDecoder
{
        ...
        out string? errorMessage)
    {
        ...
        foreach (LtbMeshTableCandidate candidate in GetLtbMeshTableCandidates(data))
        {
            if (TryParseLtbMeshes(data, candidate.FirstMeshOffset, candidate.MeshCount, out List<LithTechMesh>? meshes, out string? candidateError) &&
                meshes is not null)
            {
                TryParseLtbSkeleton(data, out List<LithTechSkeletonNode>? skeletonNodes);
                document = new LithTechModelDocument(fallbackName, meshes, storageDescription, sourceByteCount, decodedByteCount, skeletonNodes);
                return true;
            }
        ...
        out string? errorMessage)
    {
        ...
        int nodeCount = GetLtbNodeCount(data);
        ...
        out string? errorMessage)
    {
        ...
        var normals = new List<LithTechVector3>(vertexCount);
        var boneWeights = layout.IncludeWeights ? new List<LithTechVertexWeights>(vertexCount) : null;
        for (int vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++)
        {
            ...
            if (TryReadSingle(data, readPosition + layout.NormalOffset, out float normalX) &&
                TryReadSingle(data, readPosition + layout.NormalOffset + 4, out float normalY) &&
                TryReadSingle(data, readPosition + layout.NormalOffset + 8, out float normalZ))
            {
                normals.Add(new LithTechVector3(normalX, normalY, normalZ));
            }

            float weightThird = 0.0f;
            if (layout.IncludeWeights &&
                TryReadSingle(data, readPosition + layout.BoneWeightOffset, out float weightFirst) &&
                TryReadSingle(data, readPosition + layout.BoneWeightOffset + 4, out float weightSecond) &&
                (layout.ExplicitWeightCount == 2 || TryReadSingle(data, readPosition + layout.BoneWeightOffset + 8, out weightThird)))
            {
                if (layout.ExplicitWeightCount == 2)
                {
                    weightThird = Math.Max(0.0f, 1.0f - weightFirst - weightSecond);
                }

                boneWeights!.Add(new LithTechVertexWeights(weightFirst, weightSecond, weightThird));
            }
        ...
        IReadOnlyList<LithTechVertexBoneIndices>? boneIndices =
            TryReadLtbVertexBoneIndexBlocks(
                data,
                indexDataOffset + indexCount * sizeof(ushort),
                vertexCount,
                nodeCount,
                out List<LithTechVertexBoneIndices>? decodedBoneIndices)
                ? decodedBoneIndices
                : null;
        ...
        if (vertices.Count > 0 && triangleIndices.Count >= 3)
        {
            ...
            mesh = new LithTechMesh(
                ...
                null,
                normals.Count == vertices.Count ? normals : null,
                boneWeights is not null && boneWeights.Count == vertices.Count ? boneWeights : null,
                boneIndices);
        }
    ...
    private static int GetLtbVertexLookupByteCount(ReadOnlySpan<byte> data)
    {
        ...
        return (nodeCount + 1) * sizeof(uint);
    }

    private static int GetLtbNodeCount(ReadOnlySpan<byte> data)
    {
        return TryReadUInt32(data, 32, out uint nodeCount) && nodeCount is > 0 and <= 4096
            ? (int)nodeCount
            : 0;
    }

    private static bool TryParseLtbSkeleton(ReadOnlySpan<byte> data, out List<LithTechSkeletonNode>? skeletonNodes)
    {
        skeletonNodes = null;
        int expectedNodeCount = GetLtbNodeCount(data);
        if (expectedNodeCount <= 0)
        {
            return false;
        }

        byte[] rootMarker = Encoding.ASCII.GetBytes("Scene Root");
        int rootNameStart = data.IndexOf(rootMarker);
        if (rootNameStart < sizeof(ushort))
        {
            return false;
        }

        int position = rootNameStart - sizeof(ushort);
        var parsed = new List<(int Index, string Name, int ChildCount, IReadOnlyList<double> Matrix)>(expectedNodeCount);
        for (int nodeIndex = 0; nodeIndex < expectedNodeCount; nodeIndex++)
        {
            ...
            int nameStart = position + sizeof(ushort);
            int payloadStart = nameStart + nameLength;
            if (payloadStart < 0 || payloadStart + 71 > data.Length ||
                !LooksLikeLtbStringBytes(data.Slice(nameStart, nameLength)))
            {
                return false;
            }

            int index = BinaryPrimitives.ReadUInt16LittleEndian(data.Slice(payloadStart, sizeof(ushort)));
            if (index != nodeIndex)
            {
                return false;
            }
            ...
            {
                if (!TryReadSingle(data, payloadStart + 3 + matrixIndex * sizeof(float), out float value) ||
            ...
            if (!TryReadUInt32(data, payloadStart + 67, out uint childCount) || childCount > expectedNodeCount)
            {
                return false;
            }

            parsed.Add((index, DecodeLtbString(data.Slice(nameStart, nameLength)), (int)childCount, matrix));
            position = payloadStart + 71;
        }

        var parentIndices = Enumerable.Repeat(-1, expectedNodeCount).ToArray();
        var stack = new Stack<(int Index, int RemainingChildren)>();
        for (int index = 0; index < parsed.Count; index++)
        {
            ...
            if (stack.Count > 0)
            {
                (int parentIndex, int remainingChildren) = stack.Pop();
                parentIndices[index] = parentIndex;
                stack.Push((parentIndex, remainingChildren - 1));
            }

            stack.Push((index, parsed[index].ChildCount));
        }

        if (stack.Count == 0 || stack.Any(item => item.RemainingChildren != 0))
        {
            return false;
        }

        skeletonNodes = parsed
            .Select((node, index) => new LithTechSkeletonNode(index, node.Name, parentIndices[index], node.ChildCount, node.Matrix))
            .ToList();
        return true;
    }

    private static bool TryReadLtbVertexBoneIndexBlocks(
        ReadOnlySpan<byte> data,
        int position,
        int vertexCount,
        int nodeCount,
        out List<LithTechVertexBoneIndices>? boneIndices)
    {
        boneIndices = null;
        if (!TryReadUInt32(data, position, out uint sectionCount) || sectionCount == 0 || sectionCount > 4096)
        {
            return false;
        }

        long recordsEnd = (long)position + sizeof(uint) + (long)sectionCount * 12;
        if (recordsEnd > data.Length)
        {
            return false;
        }

        var decoded = Enumerable.Repeat(new LithTechVertexBoneIndices(255, 255, 255, 255), vertexCount).ToList();
        bool[] assigned = new bool[vertexCount];
        bool sawValidRecord = false;
        int recordPosition = position + sizeof(uint);
        for (int recordIndex = 0; recordIndex < sectionCount; recordIndex++)
        {
            if (!TryReadUInt16(data, recordPosition, out ushort vertexStart) ||
                !TryReadUInt16(data, recordPosition + 2, out ushort vertexRange) ||
                !TryReadUInt16(data, recordPosition + 4, out ushort packedFirstPair) ||
                !TryReadUInt16(data, recordPosition + 6, out ushort packedSecondPair))
            {
                return false;
            }

            int rangeEnd = vertexStart + vertexRange;
            if (vertexRange == 0 || rangeEnd > vertexCount)
            {
                return false;
            }

            int first = packedFirstPair & 0xFF;
            int second = packedFirstPair >> 8;
            int third = packedSecondPair & 0xFF;
            int fourth = packedSecondPair >> 8;
            foreach (int index in new[] { first, second, third, fourth })
            {
                if (index != 255 && (nodeCount <= 0 || index >= nodeCount))
                {
                    return false;
                }
            }

            var indices = new LithTechVertexBoneIndices(first, second, third, fourth);
            for (int vertexIndex = vertexStart; vertexIndex < rangeEnd; vertexIndex++)
            {
        ...
        if (!sawValidRecord || assigned.Any(value => !value))
        {
            return false;
        }
    ...
    private static bool TryCreateLtbMeshLayout(ushort meshType, out LtbMeshLayout layout)
    {
        ...
        {
            LtbMeshTypeNotSkinned => new LtbMeshLayout(ExplicitWeightCount: 0, IncludePostData: false, PostNormalByteCount: 0),
            LtbMeshTypeExtraFloat => new LtbMeshLayout(ExplicitWeightCount: 0, IncludePostData: true, PostNormalByteCount: sizeof(float)),
            LtbMeshTypeSkinned => new LtbMeshLayout(ExplicitWeightCount: 3, IncludePostData: true, PostNormalByteCount: 0),
            LtbMeshTypeSkinnedAlt => new LtbMeshLayout(ExplicitWeightCount: 2, IncludePostData: true, PostNormalByteCount: 0),
            LtbMeshTypeTwoExtraFloats => new LtbMeshLayout(ExplicitWeightCount: 0, IncludePostData: true, PostNormalByteCount: sizeof(float) * 2),
            LtbMeshTypeSkinnedExtraFloat => new LtbMeshLayout(ExplicitWeightCount: 3, IncludePostData: true, PostNormalByteCount: sizeof(float)),
            _ => default
        };
    ...
    private readonly record struct LtbMeshLayout(int ExplicitWeightCount, bool IncludePostData, int PostNormalByteCount)
    {
        public bool IncludeWeights => ExplicitWeightCount > 0;
        ...
        public int BoneWeightOffset => IncludeWeights ? 12 : -1;

        public int NormalOffset => 12 + (ExplicitWeightCount * sizeof(float));
    }
