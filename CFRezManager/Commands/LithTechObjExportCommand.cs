...
internal static class LithTechObjExportCommand
{
    ...
    private sealed record Options(string SourceRoot, string ModelQuery, string OutputPath, bool RawTransform);
    ...
    public static int Run(string[] args)
    {
        try
        {
            ...
            Console.WriteLine($"Export report: {result.ExportReportPath}");
    ...
    private static Options ParseOptions(string[] args)
    {
        ...
        for (int index = 0; index < args.Length; index++)
        {
            ...
            if (string.Equals(arg, "--raw-transform", StringComparison.OrdinalIgnoreCase))
            {
                rawTransform = true;
                continue;
            }
        ...
        return new Options(Path.GetFullPath(sourceRoot), modelQuery, Path.GetFullPath(outputPath), rawTransform);
    }
        ...
        out int skippedCount)
    {
        ...
        LithTechObjExportResult result = LithTechObjExporter.Export(options.OutputPath, sources, options.RawTransform);
        ...
        int skippedCount)
    {
        ...
        writer.WriteLine($"Raw transform: {options.RawTransform}");
        ...
        writer.WriteLine($"Export report: {result.ExportReportPath}");
