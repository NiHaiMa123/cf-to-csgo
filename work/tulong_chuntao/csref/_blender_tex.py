...
knife = objs['cf_kukri_beast']
uvl = knife.data.uv_layers['UVMap']
base_uv = [tuple(d.uv) for d in uvl.data]

# hide arms, only knife matters for texture check
objs['cf_nini_hand_gr'].hide_render = True
objs['cf_nini_arm_gr'].hide_render = True


def set_uv(fu, fv):
    for i, d in enumerate(uvl.data):
        u, v = base_uv[i]
        d.uv = (1 - u if fu else u, 1 - v if fv else v)


# knife center ~ (-2.8,-9,4.7); face normals ~ +-X
# -X side view: camera at x=-33 looking +X (sees faces pointing -X)
CAMS = {
    'Lside': Matrix([[0, 0, -1, -33], [-1, 0, 0, -9], [0, 1, 0, 4.7],
                     [0, 0, 0, 1]]),
    'Rside': Matrix([[0, 0, 1, 27], [1, 0, 0, -9], [0, 1, 0, 4.7],
                     [0, 0, 0, 1]]),
}

variants = [(False, False, 'asis'), (False, True, 'flipv'),
            (True, False, 'flipu'), (True, True, 'flipuv')]
for side, mw in CAMS.items():
    cam.matrix_world = mw
    for fu, fv, tag in variants:
        set_uv(fu, fv)
        sc.render.filepath = os.path.join(
            OUTDIR, 'blender_tex_%s_%s.png' % (side, tag))
        bpy.ops.render.render(write_still=True)
        print('rendered', side, tag)
