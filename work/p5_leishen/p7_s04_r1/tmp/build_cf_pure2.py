...
M_GUN = bpy.data.materials["CF_GunBody"]
M_PART = bpy.data.materials["CF_GunPart"]
M_ARM = bpy.data.materials["CF_Arm"]
M_HAND = bpy.data.materials["CF_Hand"]
    ...
    if bw and bi:
        ...
        for vi in range(mesh_def["vertex_count"]):
            ...
            for node_i, w in zip(ids, (w0, w1, w2, w3)):
                if node_i != 255 and w > 1e-6 and node_i < len(node_names):
                    used.add(node_i)
        ...
        for vi in range(mesh_def["vertex_count"]):
            ...
            ids = bi[4*vi:4*vi+4]
            for node_i, w in zip(ids, (w0, w1, w2, w3)):
                if node_i != 255 and w > 1e-6 and node_i in grp:
