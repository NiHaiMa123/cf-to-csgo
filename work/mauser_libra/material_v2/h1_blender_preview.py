...
MV2 = r"D:\project\cf_to_csgo\work\mauser_libra\material_v2"
...
STRATEGY = {s["material"]: {
            ...
            for s in REPORT["slots"]}
STRATEGY["cf_mauser_libra"] = {"coat": 0.30, "rough": 0.45,
                               "metal_scale": 0.3}
...
def build_mat(slot):
    p = STRATEGY.get(slot, STRATEGY["cf_mauser_libra"])
...
for o in gun:
    for i, sl in enumerate(o.material_slots):
        name = sl.material.name if sl.material else "cf_mauser_libra"
        if name in mats:
            sl.material = mats[name]
        else:
            sl.material = mats["cf_mauser_libra"]
