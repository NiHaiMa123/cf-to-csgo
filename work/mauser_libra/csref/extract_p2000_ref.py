# -*- coding: utf-8 -*-
"""Extract stock CS:GO P2000 viewmodel + soundscript (mirror of glock ref)."""
...
STEM = "v_pist_hkp2000"
MODELS = tuple(
    f"models/weapons/{STEM}.{ext}"
    for ext in ("mdl", "vvd", "dx90.vtx", "dx80.vtx", "sw.vtx", "ani")
)
SOUNDSCRIPTS = (
...
def main() -> int:
    ...
    if f.is_file():
        ...
        for m in re.finditer(r'"(Weapon_P2000[^"]*)"\s*\{([^}]*)\}', wtxt, re.I | re.S):
            name, body = m.group(1), m.group(2)
            waves = re.findall(r'"([^"]+\.wav)"', body, re.I)
            p2k_sounds[name] = {"waves": waves}
        (OUT / "p2000_soundscript.json").write_text(
            json.dumps(p2k_sounds, indent=1), encoding="utf-8")
        print(f"[ref] p2000 soundscript entries: {list(p2k_sounds)}")
    ...
    for qc in qcs:
        ...
        bones = re.findall(r'\$attachment\s+"([^"]+)"\s+"([^"]+)"', text)
        print(f"      sequences={seqs}")
        print(f"      activities={acts}")
        print(f"      attachments={bones}")
    ...
    manifest = {
        ...
        "files": extracted,
        "missing": missing,
        "sounds": list(p2k_sounds),
    }
    (OUT / "p2000_extraction_manifest.json").write_text(
