# -*- coding: utf-8 -*-
"""P3a — extract stock CS:GO glock viewmodel + soundscript."""
...
MODELS = tuple(
    f"models/weapons/v_pist_glock18.{ext}"
...
def main() -> int:
    ...
    if f.is_file():
        ...
        for m in re.finditer(r'"(Weapon_Glock[^"]*)"\s*\{([^}]*)\}', wtxt, re.I | re.S):
            name, body = m.group(1), m.group(2)
            waves = re.findall(r'"([^"]+\.wav)"', body, re.I)
            glock_sounds[name] = {"waves": waves}
        (OUT / "glock_soundscript.json").write_text(
            json.dumps(glock_sounds, indent=1), encoding="utf-8")
        print(f"[ref] glock soundscript entries: {list(glock_sounds)}")
    ...
    manifest = {
        ...
        "target": {"weapon": "glock18"},
        "files": extracted,
        "missing": missing,
        "sounds": list(glock_sounds),
    }
