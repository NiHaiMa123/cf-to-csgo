# -*- coding: utf-8 -*-
"""Find Bute Weapon records for 毛瑟-天秤座 / Mauser / Libra."""
...
OUT = Path(__file__).resolve().parent / "bute_mauser_libra.json"
...
# GBK of 毛瑟 / 天秤座 stored as latin-1 in decoded LTC
GBK_TOKENS = (
    "毛瑟".encode("gbk").decode("latin-1"),
    "天秤座".encode("gbk").decode("latin-1"),
)
KEEP_KEYS = (
    "FileName", "Sound", "Anim", "RenderStyle", "WeaponName", "StandardName",
    "GViewAnim", "Icon", "PVEffect", "Name", "Skin", "Camo", "PViewModel",
    "PViewSkin", "QV", "Model", "Texture",
)
...
def is_hit(text_upper: str, name: str, std: str, joined_gbk: str) -> bool:
    if any(t in text_upper for t in ASCII_TOKENS):
        return True
    if any(t in joined_gbk for t in ("毛瑟", "天秤座")):
        return True
    if "MAUSER" in name.upper() or "MAUSER" in std.upper():
        return True
    if "LIBRA" in name.upper() or "LIBRA" in std.upper():
        return True
    return False
